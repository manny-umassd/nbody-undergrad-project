...
def main():
    ...
    st.write("Welcome to the N-Body Simulation Dashboard! This dashboard allows you to visualize the predicted and actual positions of celestial bodies based on our Graph Neural Network (GNN) model.")
    ...
    for epoch in range(n_epochs):
        ...
        with torch.no_grad():
            ...
            st.write(f"Epoch {epoch+1}/{n_epochs}, Validation Loss: {val_loss.item():.4f}")

    # Plot training and validation loss over epochs
    fig, ax = plt.subplots()
    ax.plot(range(1, n_epochs + 1), losses, label='Training Loss')
    ax.plot(range(1, n_epochs + 1), validation_losses, linestyle='--', label='Validation Loss')
    ...
    with torch.no_grad():
        ...
        fig, ax = plt.subplots()
        for i in range(n_bodies):
            ax.scatter(actual_positions[i][0], actual_positions[i][1], color='blue', label=f'Actual Body {i+1}')
            ax.scatter(predicted_positions[i][0], predicted_positions[i][1], color='red', marker='x', label=f'Predicted Body {i+1}')
